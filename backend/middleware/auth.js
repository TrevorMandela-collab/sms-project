...
/**
 * Verifies the Bearer token on the request and attaches
 * req.user = { id, name, email, role, childIds? }
 */
...
function signToken(user) {
  return jwt.sign(
    { id: user.id, name: user.name, email: user.email, role: user.role, childIds: user.childIds || [] },
    JWT_SECRET,
    { expiresIn: '7d' }
  );
}
